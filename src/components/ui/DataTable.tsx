...
export default function DataTable<T extends object>({
  ...
  return (
    <div className="w-full p-3 overflow-hidden card">
      {/* Responsive wrapper */}
      <div className="w-full overflow-x-auto rounded-2xl border border-[var(--border)]">
        ...
        <div
          ...
          style={{
            minWidth: `${table.getAllColumns().length * 180}px`,
          }}
        >
          {/* Header */}
          <div
            ...
            style={{
              gridTemplateColumns: `repeat(${table.getAllColumns().length}, minmax(180px, 1fr))`,
              background: "var(--card)",
            }}
          >
            {table.getHeaderGroups().map((hg) =>
              hg.headers.map((h) => (
                <div
                  key={h.id}
                  className="px-4 py-3 text-xs font-bold tracking-wide uppercase border-b whitespace-nowrap"
                  style={{
          ...
          <div
            ref={parentRef}
            className={cn(
              "scrollbar overflow-y-auto overflow-x-hidden max-h-[520px]",
            )}
            style={{
            ...
            >
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <div
                    key={row.id}
                    className="grid items-center transition-colors hover:bg-black/5"
                    style={{
                      gridTemplateColumns: `repeat(${table.getAllColumns().length}, minmax(180px, 1fr))`,
                      position: "absolute",
                      top: 0,
      ...
      <div className="flex flex-col gap-3 mt-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm opacity-70">{footerText}</div>
